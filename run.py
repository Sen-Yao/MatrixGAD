...
def train(args):
    ...
    for epoch in range(args.num_epoch + 1):
        ...
        for batch_idx, item in enumerate(train_data_loader):
            ...
            emb, emb_combine, logits, outlier_emb, noised_normal_for_generation_emb, loss_rec, ortho_loss = model(concated_input_features, None,
            ...
            # 添加正交损失到总损失（使用动态权重）
            # 移除了 ring_loss
            loss = dynamic_weights['bce_loss_weight'] * loss_bce + dynamic_weights['rec_loss_weight'] * loss_rec + dynamic_weights['ortho_loss_weight'] * ortho_loss
        ...
        batched_total_loss = batched_bce_loss + batched_rec_loss + batched_ortho_loss
        end_time = time.time()
        ...
        if epoch % 10 == 0:
            ...
            with torch.no_grad():
                for _, item in enumerate(test_data_loader):
                    concated_input_features = item[0].to(device)
                    labels = item[1].to(device)
                    emb, emb_combine, logits, outlier_emb, noised_normal_for_generation_emb, loss_rec, ortho_loss = model(concated_input_features, None, None, None,
                                                                            train_flag, args)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--outlier_beta', type=float, default=0.3)  # 保留但不再使用
    parser.add_argument('--pseudo_perturbation_scale', type=float, default=1.0, help='Perturbation scale for pseudo-anomaly generation (c parameter)')
